...
バージョン1.3.5では、PICK UPおよび最新記事のカード配列の整列（高さ・タイトルの2行統一・日付の底揃え）を行い、アイキャッチ直下の不要な余白リセットおよび抜粋文の削除を行いました。提供された記事デザインに合わせた白い記事ヘッダー、680pxの本文カラム、著者・自動目次・関連記事の右カラムも備えています。PCサイドバーの各メニューは開閉でき、Codex招待ページの受付・管理者通知・再送機能も同梱しています。
